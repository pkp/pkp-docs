...
You will also see the “Start a New Submission” button under the list of Dashboard views. You can use this to make a new submission as an author. Learn about making a submission in [Learning OMP for Authors](../authors/en/).
...
**Incomplete**: The author has not completed the submission. *New in 3.5*: Press Managers and Press Editors can remove submissions with the Incomplete status by pressing the three dots next to the “Filter” button and choosing “Delete Incomplete Submissions”. 
...
**Scheduled for Publication**: The submission’s Catalog Entry has been set to a future date, and is scheduled for publication on that date.

**Published**: The submission is published and available to readers.

**Declined**: The submission was declined. Note that this status is only viewable to Press Managers and Press Editor roles. 

You can use the various views under the Dashboards menu to filter submissions by these statuses.

> Note: The “Needs Editor” and “Declined” Dashboard views are only accessible to roles with the Press Manager permission level. Series Editors see the rejected manuscripts that were assigned to them, Press Managers see all manuscripts.
{.:tip}
...
![The Filters menu of the Dashboard with options to filter by series, assigned editor, issue, category, and/or date since last activity.](./assets/filters-menu-3.5.png)
...
![The expanded Tasks menu in OMP.](./assets/tasks-3.5.png)
...
The **Marketing menu**: This is where you can add/select audience information for ONIX exports, add agents and suppliers, and configure settings for chapter publication dates.

The **Publication menu**: This is where you will make edits to information and files related to the final published version of a submission. For example, you can edit the title and abstract, list of contributors, keywords and other metadata, as well as schedule the issue for publication. This menu is covered in depth in the [Publication section of this guide](./#publication.md). You can click “Publication” to expand or collapse these options.
...
**Current Submission Language**: Indicates the current language of the submission. Useful for multilingual presses to ensure they are working in the right language.
...
**Discussions**: This panel allows participants in a stage to communicate with the author, or with others on the editorial team. Using discussions to communicate with authors, reviewers, and other members of your editorial team will be an important part of using OMP effectively, so we will cover them in-depth in [a dedicated section further below](./#discussions.md).
...
You can also click the three dots next to a participant's name to access options to Edit, Notify, Login As (this option is only accessible to Press Managers), or Remove them. 
...
Enabling this option and clicking OK would allow the author to make edits to the following sections in the Publication tab: Title & Abstract, Contributors, and Metadata.
...
The **Notes** tab of the Activity Log can be used to add editorial notes. These notes are only visible to assigned Series Editors, Press Editors, Press Managers, and Site Administrators.
...
**Edited Volume/Monograph Toggle**: Lastly, you can use the dropdown in this section to identify a submission as an edited volume or monograph.

![The location of the dropdown in OMP in the upper left of a submission record where a submission can be designated as an Edited Volume or Monograph.](./assets/type-dropdown-3.5.png)

 

Identifying a submission as an edited volume impacts the contributor list and licensing. Here’s what will change if you select edited volume:
On the contributor form, you can identify contributors as volume editors.
You can specify different licenses for each chapter.
Editors assigned to the submission will be listed in the chapter contributor list when the submission is published.

## Use Discussions to Communicate {#discussions}

OMP allows participants in each stage to use discussions to communicate about the work. Each stage has its own distinct discussion panel, and only participants assigned in that stage will have access to the stage’s discussion panel. In addition to sending messages, discussions can also be used to upload and share related files. 
...
> As a general rule, roles with Press Manager level permissions (Press Manager, Press Editor) can see all discussions in a stage. 
> Other roles, such as Series Editors, Reviewers, Authors, Copyeditors, etc., will *only see discussions where they have been selected as participants*.
...
> Creating discussions will send an email containing the message and attachment to participants. However, replying to this email from your inbox *will NOT log the communication in OMP*. If you wish to have a complete log of communication in OMP, users should be encouraged to use the Discussions panel to reply.
{:.tip}
