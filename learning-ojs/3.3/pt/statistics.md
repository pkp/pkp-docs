...
* **Visualização de registro:** na estrutura de estatísticas, qualquer referência a uma “visualização de arquivo” significa visualização ou download de um arquivo de formato específico por visitante. Também conhecido como “download da arquivo”. Se houver várias provas para um artigo \(por exemplo, arquivo HTML, PDF etc.\), cada uma terá sua própria contagem de visualizações. O OJS não faz distinção entre downloads de arquivos e visualizações na tabela de métricas, pois os downloads são contados como visualizações de usuários.
* **Múltiplos cliques:** o processo \(acidental ou nefasto\) de aumentar as contagens de uso clicando em uma página abstrata ou arquivo de prova várias vezes em rápida sucessão. OJS identifica e remove tais tentativas de suas métricas de uso de acordo com o Código de Prática do CONTADOR do Projeto.
...
Esta seção fornece estatísticas sobre o fluxo de trabalho editorial, como número de submissões recebidas, dias para a primeira decisão editorial e taxas de aceitação e rejeição. Você pode filtrar essas estatísticas usando um intervalo de datas personalizado para, por exemplo, encontrar o número de artigos aceitos durante um período de 12 meses.

![Exemplo de estatísticas editoriais.](./assets/editorial-activity.png)

Há algumas coisas importantes a serem observadas ao considerar os dados nesses relatórios:
...
* Geração de relatório personalizado: versão personalizável do relatório de estatísticas de uso, a partir da qual várias facetas podem ser selecionadas e extensões de datas específicas podem ser definidas. Consulte Gerar relatório personalizado para obter mais informações e exemplos. (Uso)
...
| 112    | Artigo    | Toronto Star demite repórter Claire Hoy, processa ele e emissoras de TV sob acusação de difamação | Vol 1, No 3 \(1974\) | CJC           | CA       | 201502  | 3         |
| 112    | Artigo    | Toronto Star demite repórter Claire Hoy, processa ele e emissoras de TV sob acusação de difamação | Vol 1, No 3 \(1974\) | CJC           | US       | 201502  | 1         |
...
As métricas são rastreadas em relação aos tipos de itens específicos (os chamados "objetos"):

* Downloads de arquivos;
* Visualizações de resumos;
* Emissão de visualizações de sumário
* Visualizações da página inicial da revista.

![A interface personalizada do gerador de relatório no OJS.](./assets/custom-report-generator1.png)
...
#### Exemplo de relatório: Qual o desempenho de determinada edição (ou seja, a mais recente) nos últimos meses?
...
![O gerador de relatórios estará personalizado com as configurações acima descritas inseridas no OJS.](./assets/custom-report-generator10.png)
...
!["Periódico" e "País" selecionados sob as opções avançadas no gerador de relatório personalizado.](./assets/custom-report-generator13.png)
...
![As configurações acima descritas estarão inseridas no OJS.](./assets/custom-report-generator7.png)

* A aba “Ordenar por” organizará seus artigos em ordem decrescente por número de downloads. Para fazer isso, selecione "Contagem" na primeira caixa suspensa e selecione "Decrescente";
