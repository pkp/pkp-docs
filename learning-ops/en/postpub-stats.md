...
It is currently not possible to withdraw a posted preprint. There are ongoing discussions to integrate an option for this for a future release. In this case, it’s recommended that the Preprint Manager replace the existing PDF with a “placeholder” PDF indicating that it has been withdrawn and the reason.
...
Click on the ‘Download Report’ button to download a CSV/Excel spreadsheet with usage statistics for preprints matching the following parameters:
...
The Editorial activity statistics provide a visual graph and trend table with a summary of the editorial activity for your Server. This can be filtered for a specific date range.
