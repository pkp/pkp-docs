...
- The XML based task schedule registration process has been [removed in favour of Laravel Task Scheduler](https://github.com/pkp/pkp-lib/issues/9678).
- The old schedule tool `tools/runScheduledTasks.php` has been removed and a new scheduling tool has been introduced as `php lib/pkp/tools/scheduler.php run` .
- A new section in the config file named `[schedule]` has been introduced to have better control of task scheduler. See [here](https://github.com/pkp/pkp-lib/issues/9678#issue-2110219392) or check the [config.TEMPLATE.inc.php](https://github.com/pkp/ojs/blob/stable-3_5_0/config.TEMPLATE.inc.php) for more details.
- [Acron](https://github.com/pkp/acron) plugin has been deprecated and removed.
- Plugins also can not register schedule tasks in XML format anymore but need to use the laravel based scheduler convention. See [here](https://github.com/pkp/pkp-lib/issues/9678#issue-2110219392) for more details and the [pln](https://github.com/pkp/pln) plugin as reference of implementation.

## Improvement of Job Runner Configuration

A new config `process_jobs_at_task_scheduler` has been introduced in `job_runner` section to [have better control of how jobs gets to processed by job runner](https://github.com/pkp/pkp-lib/issues/9823) so that a large number of jobs don't get processed at the same time by web based job runner and schedule task runner to avoid possible performance issue. Also improvement has been made to make sure that CLI based schedule task runner will also not process a large number of jobs at a time which may impact the performance of application.
