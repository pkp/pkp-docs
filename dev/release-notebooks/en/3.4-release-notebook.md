...
Plugin developers who want to use this feature should read how to [create and dispatch jobs](/dev/documentation/en/utilities-jobs).
...
Laravel's Events have been adopted to implement the observer pattern. Learn more about how to use [events and listeners](/dev/documentation/en/utilities-events).
...
- Use the form hooks to add or edit fields in an existing form. See [example](https://docs.pkp.sfu.ca/dev/plugin-guide/en/examples-custom-field) in the Plugin Guide.
...
The `Mail::send` hook was removed as part of a major update to how emails are sent. To intervene when an email is sent, add a [listener](/dev/documentation/en/utilities-events) to the `MessageSendingFromContext` and `MessageSendingFromSite` events. Learn more about the [new email system](/dev/documentation/en/utilities-email).
...
The `EditorAction::recordDecision` hook was removed as part of a major refactor of [Decisions](/dev/documentation/en/decisions). Add a [listener](/dev/documentation/en/utilities-events) to the `DecisionAdded` event to perform some action when a decision is recorded.

Several hooks which let you append data to API results have been removed. Replace these by extending the entity's [map](http://localhost:4000/dev/documentation/en/architecture-maps#using-maps).
...
Several hooks in the code that turns database results into a `DataObject` have been removed. Extend the properties of a `DataObject` by extending its [entity schema](http://localhost:4000/dev/documentation/en/architecture-entities#schema).
...
The hook `PKPLocale::installLocale` was replaced with `Locale::installLocale`.
