...
The `Mail::send` hook was removed as part of a major update to how emails are sent. Learn more about the [new email system](/dev/documentation/en/utilities-email).
...
The `EditorAction::recordDecision` hook was removed as part of a major refactor of [Decisions](/dev/documentation/en/decisions). It was replaced with an [Event](/dev/documentation/en/utilities-events). Add a listener to take action when a decision is recorded.
...
Then use the `Event` facade to subscribe the listener to the event.
...
These are the hooks that were removed.
...
Several hooks in the code that turns database results into a `DataObject` have been removed. Replace these with use of the [entity schema](http://localhost:4000/dev/documentation/en/architecture-entities#schema) to extend an entity.
