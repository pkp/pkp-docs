...
`PHPMailer` was replaced with [Laravel's Mailer](https://laravel.com/docs/9.x/mail). Test your server's email delivery after upgrading to 3.4+.

Most servers that use sendmail (default) or SMTP to send email will work without any configuration changes. However, if the sendmail binary on the server is not in its usual location, you may need to add the `sendmail_path` setting to `config.inc.php`.
...
Learn more about how to [adapt page handlers and plugins to use namespaces](https://github.com/pkp/pkp-lib/issues/6091#issuecomment-796314189).

TODO: update plugin guide and link to that instead.

## HookRegistry Renamed

The `HookRegistry` class has been renamed. Replace this code:
