...
Documentation on creating and running tests will be coming soon.
...
A new setting, `api_secret_key`, must be set in the `config.inc.php` file in order to use the API with an [API Token](http://localhost:4000/dev/api/#api-token).
...
Our new architecture for building forms is now in use. These forms can be found in the journal settings as well as the new publication forms in the worklow.
